...
@section('content')
    <form  action="{{route('products.update', $product->id)}}" method="post" enctype="multipart/form-data" >
        ...
        <div class="mb-3">
            ...
            <input type="number" name="price" class="form-control" id="exampleInputPrice" aria-describedby="PriceHelp" value="{{$product->price}}">
        </div>

        <div class="mb-3">
            <label for="exampleInputDiscount" class="form-label">Discount</label>
            <input type="number" name="discount" class="form-control" id="exampleInputDiscount" aria-describedby="PriceHelp" value="{{$product->discount}}">
        </div>

        <div class="mb-3">
            <label for="exampleInputQuantity" class="form-label">Quantity</label>
            <input type="number" name="quantity" class="form-control" id="exampleInputQuantity" aria-describedby="PriceHelp" value="{{$product->quantity}}">
        </div>

        <div class="mb-3">
            <label for="formFile" class="form-label">القسم : -</label>
            <select class="form-select" name="category_id" aria-label="Default select example">
                <option selected value="{{$product->category_id}}">{{ $product->category ? $product->category->title : 'بدون تصنيف' }}</option>
                    ...
                    <div class="tab-pane fade @if($loop->first) show active @endif"
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="{{$key}}[description]" id="exampleFormControlTextarea1" rows="3">
                                {{ $product->translate($key)->description ?? '' }}
                            </textarea>
                        </div>
